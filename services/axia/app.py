...
from utils.sanitizer import anonymize_medical_file
...
try:
    ...
    MODELS_LOADED = False
    print("[AXIA] Bypassing local PyTorch models. Using AI API purely.")
...
def classify():
    ...
    if not MODELS_LOADED:
        return jsonify(_run_llm_pipeline_for_axia(files))
...
def segment():
    ...
    if not MODELS_LOADED:
        return _mock_segment_response(files, mask_type)
...
def _run_llm_pipeline_for_axia(files) -> dict:
    import base64
    import random
    import time
    try:
        ...
        name = (f.filename or "").lower()
        if name.endswith(".dcm") or name.endswith(".dicom"):
            from utils.preprocessing import dicom_to_preview_b64
            b64_str = dicom_to_preview_b64(file_bytes)
        ...
        t0 = time.time()
        from pipeline.workflow import run_pipeline
        pipeline_res = run_pipeline(image_bytes, "axia")
        elapsed_ms = int((time.time() - t0) * 1000)

        # Default outputs if quality gate fails
        if not pipeline_res.stage1_quality_gate.passed:
            return {
                "type": "indeterminate",
                "confidence": 0.0,
                "stage1Score": 0.0,
                "stage2Score": 0.0,
                "classificationMs": elapsed_ms,
                "message": f"Quality Gate: {pipeline_res.stage1_quality_gate.reject_reason}",
                "critique": {
                    "thai_summary": f"ภาพสแกนไม่ผ่านเกณฑ์การประเมินคุณภาพ: {pipeline_res.stage1_quality_gate.reject_reason}",
                    "explainable_insights": ["ภาพถ่ายไม่สอดคล้องกับเกณฑ์ภาพถ่ายสมองทางการแพทย์ที่คมชัด"],
                    "actionable_recommendations": "โปรดอัปโหลดภาพ CT Brain ที่ถูกต้องอีกครั้ง"
                }
            }

        stage2 = pipeline_res.stage2_analysis or {}
        stage4 = pipeline_res.stage4_formatter or {}

        confidence = stage2.get("confidence", 0.5)
        finding_type = stage2.get("type") or "normal"
        
        finding_type = finding_type.lower()
        if "hemorrhage" in finding_type or "bleed" in finding_type or "ich" in finding_type:
            finding_type = "hemorrhage"
        elif "ischemic" in finding_type or "stroke" in finding_type or "infarct" in finding_type:
            finding_type = "ischemic"
        elif "normal" in finding_type or "negative" in finding_type:
            finding_type = "normal"
        else:
            finding_type = "indeterminate"

        stage1_score = confidence if finding_type == "hemorrhage" else 0.15
        stage2_score = confidence if finding_type == "ischemic" else 0.10

        thai_summary = stage4.get("primary_finding", "No acute intracranial hemorrhage or definite acute infarction identified.")
        explainable_insights = stage4.get("explainable_insights", ["The brain parenchyma appears unremarkable."])
        actionable_recommendations = stage4.get("actionable_recommendations", "Close clinical and neurological monitoring is recommended.")
        ...
        return {
            ...
            "classificationMs": elapsed_ms,
            "message": f"Analyzed using real-time LLM clinical vision pipeline.",
            "critique": {
        ...
        if t == "hemorrhage":
            return {"type": t, "confidence": c, "stage1Score": c, "stage2Score": 0.15, "classificationMs": 0, "critique": {
                "thai_summary": "Clear evidence of Intracranial Hemorrhage detected on CT image (Mock)",
                "explainable_insights": ["Hyperdense area observed in the frontal lobe", "Potential mass effect on the adjacent ventricle"],
                "actionable_recommendations": "Immediate neurosurgical consultation advised. Monitor patient's level of consciousness closely."
            }}
        if t == "ischemic":
            return {"type": t, "confidence": c, "stage1Score": 0.15, "stage2Score": c, "classificationMs": 0, "critique": {
                "thai_summary": "Suspicious for Acute Ischemic Stroke in the MCA territory (Mock)",
                "explainable_insights": ["Loss of gray-white matter differentiation observed"],
                "actionable_recommendations": "Urgent evaluation for IV tPA eligibility based on the time window."
            }}
        return {"type": t, "confidence": c, "stage1Score": 0.10, "stage2Score": 0.08, "classificationMs": 0, "message": "No CT evidence of hemorrhage; ischemia unlikely"}


def _mock_segment_response(files, mask_type) -> "flask.Response":
    import random
    import numpy as np
    import cv2
    n_files = len(files)
    ...
    for i in range(n_files):
        # Middle 30% of slices get a mask
        mask_found = i >= int(n_files * 0.35) and i <= int(n_files * 0.65)
        mask_image = None
        if mask_found:
            mask_arr = np.zeros((256, 256), dtype=np.float32)
            cv2.circle(mask_arr, (150, 110), 25, 1.0, -1)
            # Add blur to make it natural
            mask_arr = cv2.GaussianBlur(mask_arr, (5, 5), 0)
            mask_arr = (mask_arr > 0.3).astype(np.float32)
            from utils.mask_analysis import mask_to_overlay_b64
            mask_image = mask_to_overlay_b64(mask_arr, mask_type)
            
        slice_results.append({
            "maskFound": mask_found,
            "confidence": round(random.uniform(0.78, 0.94), 4) if mask_found else 0.0,
            "maskImage": mask_image
        })
        
    return jsonify({
        "maskFound":      any(sr["maskFound"] for sr in slice_results),
        "volume":         32.4 if mask_type == "hemorrhage" else None,
        "midlineShift":   4.2 if mask_type == "hemorrhage" else None,
...
# ---------------------------------------------------------------------------
# Core Gateway Orchestration Endpoints
# ---------------------------------------------------------------------------

@app.route("/api/core/orchestrate", methods=["POST"])
def orchestrate():
    """APEX 4-Stage Core Orchestrator"""
    if not os.environ.get("OPENAI_API_KEY") and not os.environ.get("GROQ_API_KEY") and not os.environ.get("LITELLM_KEY"):
        # If no keys are configured, return 503 so the frontend falls back to Demo Mode
        return jsonify({"error": "AI APIs not configured. Falling back to Demo Mode."}), 503
        
    f = request.files.get("file")
    mrn = request.form.get("mrn", "UNKNOWN")
    target_module = request.form.get("target_module", "axia")
    
    if not f:
        return jsonify({"error": "No file provided. PACS integration not mocked in backend yet."}), 400
    ...
    try:
        ...
        # 1. Sanitize Data
        anonymized_bytes, anon_id = anonymize_medical_file(file_bytes, filename)
        
        # 2. Run Pipeline
        pipeline_res = run_pipeline(anonymized_bytes, target_module)
        
        return jsonify({
            "pipeline_results": pipeline_res.model_dump(),
            "anon_image_id": anon_id,
            "status": "success"
        })
    except Exception as e:
        import traceback
...
@app.route("/api/core/audit", methods=["POST"])
def audit():
    ...
    try:
        log_id = log_audit(
            ...
            anon_image_id=data.get("anon_image_id", "UNKNOWN"),
