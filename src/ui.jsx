...
function Icon({ name, size = 16, className = "", strokeWidth = 1.75, style }) {
  ...
  useEffect(() => {
    ...
    const svg = node ? window.lucide.createElement(node) : null;
    if (svg) {
  ...
  return <span ref={ref} className="inline-flex shrink-0" style={style} />;
}
...
// ---- Confirm Modal (for dangerous / irreversible actions) ----
function ConfirmModal({ title, body, confirmLabel = "Confirm", cancelLabel = "Cancel", onConfirm, onClose, danger = false }) {
  useEffect(() => {
    const onKey = (e) => e.key === "Escape" && onClose?.();
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(10,19,34,0.45)" }} onClick={onClose}>
      <div className="card card-elev w-full" style={{ maxWidth: 400 }} onClick={(e) => e.stopPropagation()}>
        <div className="px-4 py-3 border-b hairline flex items-center gap-3">
          <span className={`w-9 h-9 rounded-md grid place-items-center shrink-0 ${danger ? "bg-[#fde8e9] text-[#8b1a1f]" : "bg-slate-100 text-slate-600"}`}>
            <Icon name={danger ? "alert-triangle" : "info"} size={17} />
          </span>
          <div className="text-[14px] font-semibold t-primary">{title}</div>
          <button onClick={onClose} className="ml-auto w-8 h-8 rounded-md grid place-items-center hover:bg-slate-100 t-secondary" title="Close (Esc)"><Icon name="x" size={14} /></button>
        </div>
        <div className="p-4">
          <p className="text-[13px] t-secondary leading-relaxed">{body}</p>
          <div className="mt-4 pt-3 border-t hairline flex items-center justify-end gap-2">
            <Btn variant="ghost" size="sm" onClick={onClose}>{cancelLabel}</Btn>
            <Btn variant={danger ? "danger" : "primary"} size="sm" icon={danger ? "alert-triangle" : "check"} onClick={() => { onConfirm?.(); onClose?.(); }}>{confirmLabel}</Btn>
          </div>
          <div className="mt-2 text-[11px] t-muted flex items-center gap-1"><Icon name="shield" size={11} />This action is logged to the audit trail.</div>
        </div>
      </div>
    </div>
  );
...
Object.assign(window, {
  ...
  applyAccent, Tip, InfoTip, AIOverride, Modal, ConfirmModal, MfaInput, ToastProvider, useToasts,
  fmtTime, FLAG_DEFS,
});
