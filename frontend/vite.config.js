...
export default defineConfig({
  ...
  server: {
    ...
    proxy: {
      ...
      // PICHA AI — MARS 7-Agent Pipeline (FastAPI :8200, SSE streaming)
      // NOTE: must be listed before '/api/picha' to avoid prefix collision
      "/api/picha-ai": {
        target: "http://127.0.0.1:8200",
        changeOrigin: true,
        rewrite: (path) => path.replace(/^\/api\/picha-ai/, ""),
        timeout: 600_000,
      },
