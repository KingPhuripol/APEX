...
const AGENT_META = {
  MLPrescreen: {
    icon: Microscope,
    label: "ML Pre-screen",
    labelTH: "Tissue Composition Pre-screen",
    color: "text-purple-400",
    bg: "bg-purple-500/10",
    border: "border-purple-500/30",
    note: "Model trained on CRC-NCT-HE-555K (colorectal), not a CCA-specific dataset · accuracy 76.33%",
  },
  SlideQCAgent: {
    icon: ScanSearch,
    label: "Slide QC",
    labelTH: "Slide Quality Control",
    color: "text-blue-400",
    bg: "bg-blue-500/10",
    border: "border-blue-500/30",
    note: "If QC fails, recommend re-staining or re-scanning the slide.",
  },
  ParasitologistAgent: {
    icon: Bug,
    label: "Parasitologist",
    labelTH: "OV Parasitology Assessment",
    color: "text-emerald-400",
    bg: "bg-emerald-500/10",
    border: "border-emerald-500/30",
    note: "OV-associated CCA is commonly found at the peri-hilar location.",
  },
  GradingAgent: {
    icon: BarChart3,
    label: "WHO Grading",
    labelTH: "WHO Histological Grading",
    color: "text-yellow-400",
    bg: "bg-yellow-500/10",
    border: "border-yellow-500/30",
    note: "Grade G3 is associated with significantly worse overall survival.",
  },
  SpatialAgent: {
    icon: Map,
    label: "Spatial Analysis",
    labelTH: "Spatial Microenvironment Analysis",
    color: "text-orange-400",
    bg: "bg-orange-500/10",
    border: "border-orange-500/30",
    note: "LVI and PNI are independent adverse prognostic factors.",
  },
  OncologistAgent: {
    icon: Hospital,
    label: "Oncologist",
    labelTH: "AJCC Staging & Treatment Planning",
    color: "text-red-400",
    bg: "bg-red-500/10",
    border: "border-red-500/30",
    note: "Results should be discussed at a multidisciplinary tumor board (MDT).",
  },
  TimeMachineAgent: {
    icon: Timer,
    label: "Time Machine",
    labelTH: "Survival Prognosis Modelling",
    color: "text-pink-400",
    bg: "bg-pink-500/10",
    border: "border-pink-500/30",
    note: "Survival estimates are population-level statistics, not individual-level predictions.",
  },
  ReportAgent: {
    icon: FileText,
    label: "Report Compiler",
    labelTH: "CAP Protocol Report Compilation",
    color: "text-cyan-400",
    bg: "bg-cyan-500/10",
    border: "border-cyan-500/30",
    note: "Must be reviewed and signed by a licensed pathologist before clinical use.",
  },
};
...
function Row({ label, value, accent }) {
  return (
    <div className="flex justify-between items-start py-1 border-b border-[var(--line)] last:border-0">
      ...
      <span
        className={`text-xs font-medium text-right max-w-[60%] ${accent || "text-[var(--text)]"}`}
      >
        {value}
      </span>
    </div>
...
// ── Main Component ──────────────────────────────────────────────────────
export default function PICHAClinicalReport({
  report = {},
  events = [],
  slideImageUrl,
}) {
  ...
  return (
    <div className="space-y-3 animate-fade-in">
        ...
        <div className="flex items-start gap-2.5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-[var(--text-2)] leading-relaxed">
              ...
              <strong className="text-[var(--text)]">PICHA MARS</strong> — an
              AI-assisted clinical decision support tool. Data is processed in
              compliance with applicable personal data protection regulations.
              All findings{" "}
              <strong className="text-[var(--text)]">
                must be reviewed and signed by a licensed specialist physician
              </strong>{" "}
              before clinical use.
            </p>
          </div>
      ...
      {/* ── AI-Analyzed Slide Image ── */}
      {slideImageUrl && (
        <div className="border border-[var(--line)] bg-[var(--surface-2)] rounded-xl overflow-hidden">
          <div className="flex items-center gap-2 px-4 py-2 border-b border-[var(--line)]">
            ...
            <p className="text-[11px] font-bold uppercase tracking-wider text-[var(--muted)]">
              AI-Analyzed Histopathology Slide
            </p>
          </div>
          <div className="relative">
            <img
              src={slideImageUrl}
              alt="Histopathology slide analyzed by MARS"
              className="w-full max-h-64 object-contain bg-black"
              style={{ display: "block" }}
            />
            {/* Overlay remark */}
            <div className="absolute top-2 left-2 flex items-center gap-1.5 px-2.5 py-1 rounded text-[11px] font-bold text-white bg-violet-700 shadow-lg">
              <span>MARS AI Finding:</span>
              <span>{diagnosis !== "—" ? diagnosis : "Pending"}</span>
              {confidence > 0 && (
            ...
            {stage !== "—" && (
              <div className="absolute bottom-2 right-2 px-2 py-0.5 rounded text-[10px] font-bold text-white bg-black/70 border border-white/20">
                {stage}
              </div>
            )}
          </div>
          ...
          <div className="px-4 py-2 space-y-0.5">
            ...
            {isOv && (
              <p className="text-[11px] text-emerald-400 font-semibold">
                ◦ OV-associated cholangiocarcinoma detected
              </p>
            )}
          </div>
        </div>
      ...
      <div className="bg-gradient-to-r from-violet-500/10 to-transparent border border-violet-500/30 rounded-xl px-5 py-4">
        <p className="text-[10px] uppercase tracking-widest text-violet-400 mb-1.5">
          Primary Diagnosis
        </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <Section title="Slide Quality Control">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span
              className={`text-sm font-bold ${qcProceed ? "text-emerald-400" : "text-red-400"}`}
            >
              {qcQuality}
            </span>
          </div>
        ...
        <Section title="Spatial / Microenvironment">
          ...
          <Row
            ...
            value={safeStr(sp.lymphovascular_invasion)}
            accent={
              sp.lymphovascular_invasion === "Present"
          ...
          <Row
            ...
            value={safeStr(sp.perineural_invasion)}
            accent={
              sp.perineural_invasion === "Present"
                ? "text-red-400 font-bold"
                : undefined
            }
          />
                  ...
                  <div key={agentKey}>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-8 h-8 rounded-full ${meta.bg} flex items-center justify-center shrink-0`}
                        >
