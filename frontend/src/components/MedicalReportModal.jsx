...
import { Printer, X, ShieldCheck, Activity, User, Calendar, FileText } from 'lucide-react';

export default function MedicalReportModal({ isOpen, onClose, analysisResult, patientId, moduleName }) {
  ...
  const primaryFinding = stage4_formatter?.primary_finding || 'No primary finding available.';
  const insights = stage4_formatter?.explainable_insights || [];
  const recommendations = stage4_formatter?.actionable_recommendations || 'No recommendations provided.';
  ...
  const getModuleTitle = () => {
    switch(moduleName) {
      case 'axia': return 'Neuroradiology CT Report';
      case 'picha': return 'Gastrointestinal Pathology Report';
      case 'smartliva': return 'Hepatobiliary Ultrasound Report';
      default: return 'Clinical AI Analysis Report';
    }
  };
      ...
      <div className="relative w-full max-w-4xl max-h-[90vh] flex flex-col bg-white rounded-xl shadow-2xl overflow-hidden print:w-full print:max-w-none print:max-h-none print:shadow-none print:rounded-none">
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-gray-50 border-b border-gray-200 print:hidden">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-lg font-bold text-gray-800">Finalized Medical Report</h2>
          </div>
          ...
          <div className="max-w-3xl mx-auto space-y-8 text-gray-900">
            ...
            <div className="grid grid-cols-2 gap-6 bg-gray-50 p-5 rounded-lg border border-gray-200">
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Patient ID</p>
                  <p className="text-lg font-mono font-bold text-gray-900">{patientId || 'UNKNOWN'}</p>
                </div>
              </div>
              <div className="flex items-start space-x-3">
                <Calendar className="w-5 h-5 text-gray-400 mt-0.5" />
                <div>
                  <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Analysis Date</p>
            ...
            <div className="space-y-6">
              ...
              <section>
                <h3 className="text-sm font-bold text-gray-500 uppercase tracking-widest border-b border-gray-200 pb-2 mb-4">
                  {stage4_formatter?.synoptic_report ? 'Microscopic Description' : 'Clinical Evidence & Insights'}
                </h3>
                  ...
                  {insights.length > 0 ? (
                    insights.map((insight, idx) => (
                      <li key={idx} className="flex items-start">
                        ...
                        <span className="text-base text-gray-800 leading-relaxed">{insight}</span>
                      </li>
                    ))
                  ) : (
                    <li className="text-gray-500 italic">No detailed insights provided.</li>
                  )}
                </ul>
              </section>
                  ...
                  <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
                    <table className="w-full text-left border-collapse">
                      <tbody className="divide-y divide-gray-200">
                        {Object.entries(stage4_formatter.synoptic_report).map(([key, val]) => (
                          <tr key={key} className="even:bg-gray-50">
                            <td className="py-3 px-4 text-sm font-semibold text-gray-700 w-1/3 align-top">{key}</td>
                            <td className="py-3 px-4 text-base text-gray-900 font-medium">{val}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              <div className="flex justify-between items-end">
                <div className="max-w-md">
                  <p className="text-[10px] text-gray-500 leading-relaxed">
                    <strong>DISCLAIMER:</strong> This report is generated by the APEX Clinical AI Platform (Multi-Agent Reasoning System). 
                    It is designed to assist, not replace, clinical judgment. Final diagnostic and treatment 
                    decisions must be made by a qualified healthcare professional.
                  </p>
                </div>
                <div className="text-center w-64">
                  <div className="border-b border-gray-400 h-10 mb-2"></div>
                  <p className="text-sm font-bold text-gray-900">Electronically Signed</p>
                  <p className="text-xs text-gray-500 mt-1">Attending Physician / Specialist</p>
                </div>
              </div>
            </div>
