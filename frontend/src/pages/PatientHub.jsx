...
export default function PatientHub() {
        ...
        return (
          <div className="h-full flex flex-col items-center justify-center text-center">
            ...
            <p className="text-[var(--text-2)] max-w-md text-sm">
              Select a diagnostic module from the left menu to begin AI-assisted
              analysis for {patientName}.
            </p>
          </div>
        );
    }
  ...
  return (
    <div
      className="flex h-full flex-col bg-[var(--bg)] transition-colors duration-300"
      style={getModuleTheme(module)}
    >
      {/* Patient Header */}
      ...
      <div className="flex flex-col md:flex-row flex-1 min-h-0">
        ...
        <div className="w-full md:w-56 bg-[var(--surface-2)] border-b md:border-b-0 md:border-r border-[var(--line)] p-2 md:p-3 flex flex-row md:flex-col space-x-2 md:space-x-0 md:space-y-1.5 z-10 overflow-x-auto md:overflow-y-auto shrink-0">
          <h3 className="hidden md:block text-[10px] font-bold text-[var(--muted)] uppercase tracking-widest mb-1.5 px-2 mt-2">
            AI Diagnostics
          </h3>
          ...
          >
            <div
              className={`p-1.5 rounded-md mr-2 md:mr-2.5 ${module === "axia" ? "bg-[var(--info-soft)]" : "bg-[var(--surface)]"}`}
            >
              <Brain
                className={`w-4 h-4 ${module === "axia" ? "text-[var(--info)]" : "text-[var(--muted)]"}`}
              />
            </div>
          ...
          >
            <div
              className={`p-1.5 rounded-md mr-2 md:mr-2.5 ${module === "smartliva" ? "bg-[var(--info-soft)]" : "bg-[var(--surface)]"}`}
            >
              <LiverIcon
                className={`w-4 h-4 ${module === "smartliva" ? "text-[var(--info)]" : "text-[var(--muted)]"}`}
              />
            </div>
            <div className="flex-1">
          ...
          >
            <div
              className={`p-1.5 rounded-md mr-2 md:mr-2.5 ${module === "picha" ? "bg-[var(--info-soft)]" : "bg-[var(--surface)]"}`}
            >
              <Microscope
                className={`w-4 h-4 ${module === "picha" ? "text-[var(--info)]" : "text-[var(--muted)]"}`}
              />
            </div>
            <div className="flex-1">
