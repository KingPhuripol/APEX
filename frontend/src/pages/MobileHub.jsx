...
const MOCK_ALERTS = [
  {
    id: "alt-1",
    mrn: "HN-2566-4891",
    patientName: "Somchai Jaidee, 67M",
    module: "axia",
    ...
    insights: [
      ...
      "Midline shift of 4.8 mm toward the right hemisphere",
      "Estimated hematoma volume: 34.2 mL",
    ],
    recommendation:
      "Urgent neurosurgical consultation required. Assess ICP and consider emergent evacuation.",
  },
  {
    id: "alt-2",
    mrn: "HN-2566-5512",
    patientName: "Pornchai Srisuk, 52M",
    module: "smartliva",
    finding: "Severe Liver Fibrosis (F4 — Cirrhosis suspected)",
    severity: "high",
    time: "15 mins ago",
    insights: [
      "Coarse parenchymal texture with nodular surface contour",
      "Portal vein diameter enlarged at 13.5 mm — portal hypertension risk",
      "FibroScan stiffness pattern consistent with F4 cirrhosis",
    ],
    recommendation:
      "Monitor for portal hypertension complications and HCC surveillance every 6 months.",
  },
  {
    id: "alt-3",
    mrn: "HN-2566-9341",
    patientName: "Prayoon Chaidee, 58M",
    module: "picha",
    finding: "Intrahepatic Cholangiocarcinoma (iCCA)",
    severity: "high",
    time: "1 hr ago",
    insights: [
      "Grade 3 poorly differentiated cholangiocarcinoma confirmed",
      "Positive anterior resection margin (< 1 mm)",
      "OV-associated background — chronic opisthorchiasis history",
    ],
    recommendation:
      "MDT review for adjuvant Gemcitabine/Cisplatin chemotherapy planning.",
  },
];
...
export default function MobileHub() {
  ...
  const [patientChecklist, setPatientChecklist] = useState([
    { id: "water", label: "Drink 8 glasses of water (2 litres)", done: true },
    { id: "diet", label: "Limit salt and high-fat foods", done: false },
    { id: "exercise", label: "Light walk for 20 minutes", done: false },
    { id: "meds", label: "Take medication as prescribed", done: true },
  ]);
    ...
    hepasage: [
      {
        role: "assistant",
        content:
          "**HepaSage AI** online. I can help interpret liver ultrasound results, fibrosis staging, and clinical follow-up recommendations. How can I assist?",
      },
    ],
    hepasagePatient: [
      {
        role: "assistant",
        content:
          "**HepaSage** here. Welcome back! How are you feeling today? I can help with liver health tips, your diet plan, and medication reminders. Just ask.",
      },
    ],
    mars: [
      {
        role: "assistant",
        content:
          "MARS Pathology Agent initialized. Ask me about digital pathology slide tiles, Opisthorchis viverrini screening, or cholangiocarcinoma classifications.",
      },
    ],
  });
  ...
  const runAiAnalysis = async () => {
    ...
    try {
      if (selectedModule === "smartliva") {
        const res = await smartlivaPredict(files[0], { language: "en" });
        await delay(1000);
        setScanResult({
          ...
          riskLevel:
            res.risk_level === "High"
              ...
              : res.risk_level === "Moderate"
                ? "moderate"
                : "normal",
          thaiSummary: res.fibrosis_text || "Liver tissue assessment complete.",
          explainableInsights: [
            res.stiffness_status
              ? `Tissue stiffness: ${res.stiffness_status}`
              : "Fibrosis pattern data accumulated.",
            `Steatosis status: ${res.steatosis_status}`,
            `Model confidence: ${(res.fibrosis_confidence * 100).toFixed(1)}%`,
          ],
          actionableRecommendations:
            res.recommendation || "Follow up as directed by your specialist.",
          rawResponse: res,
        });
        ...
        let risk = "normal";
        let rec = "Follow up as per general clinical indication.";
        if (clf.type === "hemorrhage") {
          risk = "critical";
          rec = "Urgent neurosurgical consult. Monitor ICP and consider hematoma evacuation.";
        } else if (clf.type === "ischemic") {
          risk = "high";
          rec = "Assess tPA eligibility and monitor for limb weakness or speech deficits.";
        }
        ...
        setScanResult({
          ...
          title:
            clf.type === "hemorrhage"
              ...
              : clf.type === "ischemic"
                ? "Acute Ischemia Detected"
                : "No Acute Intracranial Pathology",
          riskLevel: risk,
          thaiSummary:
            clf.critique?.summary ||
            (clf.type === "hemorrhage"
              ? "Intracranial hemorrhage confirmed on CT."
              : clf.type === "ischemic"
                ? "Acute ischemic stroke pattern suspected."
                : "Brain CT within normal limits."),
          explainableInsights: clf.critique?.explainable_insights || [
            `Stage 1 abnormality score: ${(clf.stage1Score * 100).toFixed(1)}%`,
            `Stage 2 abnormality score: ${(clf.stage2Score * 100).toFixed(1)}%`,
          ],
          actionableRecommendations:
            clf.critique?.actionable_recommendations || rec,
        });
      } else {
        // PICHA Pathology
        await delay(1500);
        setScanResult({
          ...
          riskLevel: "high",
          thaiSummary:
            "Intrahepatic Cholangiocarcinoma confirmed — Grade 3, poorly differentiated.",
          explainableInsights: [
            "Nuclear pleomorphism Grade 3 identified in bile duct cells",
            "Positive anterior resection margin (< 1 mm)",
            "24-month recurrence risk estimated at 64%",
          ],
          actionableRecommendations:
            "MDT referral for adjuvant Gemcitabine/Cisplatin chemotherapy planning.",
        });
      }
      setScanStatus("done");
    } catch (err) {
      console.error(err);
      setScanError(err.message || "AI processing connection failed. Please try again.");
      setScanStatus("error");
    }
  };
  ...
  const sendChatMessage = async () => {
    ...
    try {
      ...
      if (chatBot === "hepasage" || chatBot === "hepasagePatient") {
        ...
        const activeHistory = isPatient
          ? messages.hepasagePatient
          : messages.hepasage;

        let apiHistory = activeHistory.map((m) => ({
          role: m.role,
          content: m.content,
        }));
        if (isPatient) {
          const instructions = ` (Please respond warmly as a personal liver health coach. Use simple, encouraging English. Avoid heavy medical jargon.)`;
          apiHistory.push({
            role: "user",
            content: userMsg.content + instructions,
          });
        } else {
          apiHistory.push(userMsg);
        }

        const res = await smartlivaChat(apiHistory, "en");
        reply = res.reply;
      } else {
      ...
      <div className="w-full max-w-md bg-[#0c1424] min-h-screen flex flex-col relative shadow-2xl border-x border-[#1a2d4c]">
        ...
        <header className="h-14 border-b border-[#1c3052] bg-[#0c1424]/90 backdrop-blur-md sticky top-0 z-30 flex items-center justify-between px-4">
          <div className="flex items-center space-x-2">
            <span
              className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-white shadow-md transition-colors ${
                persona === "doctor"
            ...
            <div>
              <h1 className="font-bold text-sm tracking-wide">
                {persona === "doctor" ? "APEX Clinic" : "APEX Patient"}
              </h1>
              <p className="text-[10px] text-blue-400 font-medium">
                {persona === "doctor"
                  ? "Decision Companion"
                  : "Liver Health Companion"}
              </p>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            <button
              onClick={() =>
                setPersona((prev) => (prev === "doctor" ? "patient" : "doctor"))
              }
            ...
            >
              {persona === "doctor" ? "Doctor Mode" : "Patient Mode"}
            </button>
          </div>
        </header>
        ...
        <main className="flex-1 overflow-y-auto pb-20 px-4 pt-4">
            ...
            (persona === "doctor" ? (
              <div className="space-y-4 fade-up">
                <div className="flex justify-between items-center mb-2">
                  <h2 className="font-bold text-xs uppercase tracking-wider text-[#7a8dae]">
                    Recent Priority Alarms
                  </h2>
              ...
              <div className="space-y-4 fade-up">
                ...
                <div className="bg-gradient-to-r from-emerald-950/40 to-teal-950/30 border border-emerald-500/20 rounded-2xl p-4">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-[#7a8dae]">
                        HN 3041-558 • อายุ 52 ปี
                      </p>
                    </div>
                  </div>
                ...
                <div className="bg-[#111c34] border border-[#1d335c] rounded-2xl p-4">
                  <h3 className="font-bold text-xs uppercase tracking-wider text-[#7a8dae] mb-3">
                    เป้าหมายสุขภาพวันนี้
                  </h3>
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-xs text-[#a0aec0]">
                      ความร่วมมือในการดูแลตัวเอง
                    </span>
                    <span className="text-xs font-bold text-emerald-400">
                      {Math.round(
                        ((patientChecklist.filter((t) => t.done).length +
                          (waterIntake >= 8 ? 1 : 0)) /
                          5) *
                          100,
                      )}
                  ...
                  <div className="w-full bg-[#0c1424] h-2 rounded-full overflow-hidden border border-[#1c3052]">
                    <div
                      ...
                      style={{
                        width: `${((patientChecklist.filter((t) => t.done).length + (waterIntake >= 8 ? 1 : 0)) / 5) * 100}%`,
                      }}
                    />
                  </div>
                </div>
                ...
                <div className="bg-[#111c34] border border-[#1d335c] rounded-2xl p-4 space-y-3">
                  <h3 className="font-bold text-xs uppercase tracking-wider text-[#7a8dae] mb-1">
                    บันทึกภารกิจประจำวัน
                  </h3>
                  {patientChecklist.map((item) => (
                    ...
                    >
                      ...
                      >
                        {item.done && (
                          <CheckCircle2 className="w-3.5 h-3.5 shrink-0 text-white fill-emerald-500" />
                        )}
                      </span>
                      <span
                        className={`text-xs text-left ${item.done ? "line-through text-[#7a8dae]" : "text-white"}`}
                      >
                        {item.label}
                      </span>
                    </div>
                ...
                <div className="bg-[#111c34] border border-[#1d335c] rounded-2xl p-4">
                  <div className="flex justify-between items-center mb-2">
                    <div>
                      <h3 className="font-bold text-xs uppercase tracking-wider text-[#7a8dae]">
                        เครื่องมือบันทึกน้ำดื่ม
                      </h3>
                      <p className="text-[10px] text-[#7a8dae] mt-0.5">
                        ช่วยลดภาระการทำงานหนักของตับ
                      </p>
                    </div>
                  ...
                  <div className="flex space-x-2">
                    ...
                    <button
                      onClick={() =>
                        setWaterIntake((prev) => Math.min(8, prev + 1))
                      }
                ...
                <div className="bg-gradient-to-r from-blue-950/30 to-indigo-950/20 border border-blue-500/20 rounded-2xl p-4 flex items-start space-x-3">
                  ...
                  <div className="space-y-1">
                    <h4 className="font-bold text-xs text-white">
                      นัดพบแพทย์ติดตามผลครั้งถัดไป
                    </h4>
                    <p className="text-xs text-[#a0aec0]">
                      วันศุกร์ที่ 12 มิถุนายน 2569 เวลา 09:00 น.
                      ที่คลินิกตับและทางเดินอาหาร ชั้น 4
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-4 fade-up">
              <div className="text-center mb-4">
                <h2 className="font-bold text-sm text-white">
                  {persona === "doctor"
                    ? "Camera Diagnostic"
                    : "เครื่องแปลผลแล็บอัจฉริยะ"}
                </h2>
                <p className="text-xs text-[#7a8dae]">
                  {persona === "doctor"
                    ? "Select module and snap a medical image"
                    : "ถ่ายภาพใบรายงานผลแพทย์หรือฟิล์มอัลตราซาวด์ตับ"}
                </p>
              </div>
              ...
              <div className="bg-[#111c34] border border-[#1d335c] rounded-xl p-6 flex flex-col items-center justify-center text-center relative overflow-hidden min-h-[220px]">
                {previewUrl ? (
                  <div className="relative w-full h-full flex flex-col items-center">
                    <img
                      src={previewUrl}
                      alt="Preview"
                      className="max-h-[160px] object-contain rounded-lg filter grayscale mb-4"
                    />
                    {scanStatus === "idle" && (
                      <button
                        onClick={() => {
                  ...
                  <>
                    <UploadCloud
                      className={`w-12 h-12 mb-3 ${persona === "doctor" ? "text-blue-500/60" : "text-emerald-500/60"}`}
                    />
                    <h3 className="font-bold text-sm text-white mb-1">
                      {persona === "doctor"
                        ? "Upload Patient Scan"
                        : "อัปโหลดใบตรวจสุขภาพตับ"}
                    </h3>
                    <p className="text-xs text-[#7a8dae] mb-4">
                      {persona === "doctor"
                        ? "Tap to take photo or choose file"
                        : "ถ่ายรูปด้วยกล้อง หรือเลือกไฟล์จากแกลเลอรี"}
                    </p>
                    <input
                      type="file"
                ...
                {scanStatus === "checking" && (
                  <div className="absolute inset-0 bg-[#0c1424]/90 backdrop-blur-sm flex flex-col items-center justify-center text-center">
                    <Loader2
                      className={`w-8 h-8 animate-spin mb-3 ${persona === "doctor" ? "text-blue-500" : "text-emerald-500"}`}
                    />
                    <div className="font-bold text-sm text-white">
                      {persona === "doctor"
                        ? "Stage 1: Quality Gate"
                        : "ขั้นตอนที่ 1: ตรวจสอบความละเอียด"}
                    </div>
                    <div className="text-[11px] text-[#7a8dae] mt-1">
                      {persona === "doctor"
                        ? "Validating medical imaging fidelity..."
                        : "ระบบกำลังปรับความคมชัดภาพผลตรวจแล็บตับ..."}
                    </div>
                  </div>
                )}
                {scanStatus === "analyzing" && (
                  <div className="absolute inset-0 bg-[#0c1424]/90 backdrop-blur-sm flex flex-col items-center justify-center text-center">
                    ...
                    <div className="font-bold text-sm text-white">
                      {persona === "doctor"
                        ? "Stage 2 & 3: Clinical Auditor"
                        : "ขั้นตอนที่ 2: โค้ช AI แปลผลอัจฉริยะ"}
                    </div>
                    <div className="text-[11px] text-[#7a8dae] mt-1">
                      {persona === "doctor"
                        ? "Running CoT Vision API & safety critique..."
                        : "กำลังวิเคราะห์คำย่อแพทย์และสถิติตับอย่างเป็นมิตร..."}
                    </div>
                  </div>
                )}
              </div>
                  ...
                  {persona === "doctor" ? (
                    <>
                      ...
                      <div className="bg-[#111c34] border-l-4 border-blue-500 p-4 rounded-r-xl border-y border-r border-[#1d335c] flex justify-between items-start">
                        <div>
                          <span
                            className={`text-[8px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider bg-red-500/20 text-red-400 border border-red-500/30`}
                          >
                      ...
                      <div className="bg-[#111c34] border border-[#1d335c] rounded-xl overflow-hidden">
                        <div className="bg-[#152444] px-4 py-2 border-b border-[#1d335c] flex justify-between items-center">
                          <span className="text-[10px] font-bold text-white uppercase tracking-wider">
                            รายงานผลทางการแพทย์ (ไทย)
                          </span>
                          <span className="text-[9px] bg-blue-600/20 text-blue-400 border border-blue-600/30 px-2 py-0.5 rounded font-bold">
                            AI Synthesized
                          </span>
                        </div>
                        <div className="p-4 space-y-4">
                          <div>
                            <div className="text-[9px] uppercase text-[#7a8dae] font-bold mb-1">
                              บทสรุปผลการตรวจ
                            </div>
                          ...
                          <div>
                            <div className="text-[9px] uppercase text-[#7a8dae] font-bold mb-1">
                              ลักษณะทางภาพถ่ายที่สำคัญ (Insights)
                            </div>
                    ...
                    <>
                      ...
                      <div className="bg-gradient-to-r from-emerald-950/40 to-teal-950/30 border-l-4 border-emerald-500 p-4 rounded-r-xl border-y border-r border-emerald-500/20 flex justify-between items-start">
                        <div>
                          ...
                          <h3 className="font-bold text-sm text-white mt-1.5">
                            ผลแปลรายงานตรวจสุขภาพตับของคุณสุรเดช
                          </h3>
                          <p className="text-[10px] text-emerald-400 mt-1">
                            วิเคราะห์โดย: ระบบโค้ชตับ AI ปัญญาประดิษฐ์
                          </p>
                        </div>
                        <button
                          onClick={() => {
                      ...
                      <div className="bg-[#111c34] border border-[#1d335c] rounded-xl overflow-hidden">
                        <div className="bg-[#152444] px-4 py-2.5 border-b border-[#1d335c] flex justify-between items-center">
                          <span className="text-[10px] font-bold text-white uppercase tracking-wider">
                            แผงความรู้และคำอธิบายตับอย่างง่าย
                          </span>
                          <span className="text-[9px] bg-emerald-600/20 text-emerald-400 border border-emerald-600/30 px-2 py-0.5 rounded font-bold">
                            เข้าใจง่ายสำหรับคนไข้
                          </span>
                        </div>
                        <div className="p-4 space-y-4">
                          <div>
                            <div className="text-[9px] uppercase text-emerald-400 font-bold mb-1">
                              สรุปภาพรวมร่างกายวันนี้
                            </div>
                            <p className="text-xs text-white leading-relaxed font-medium">
                          ...
                          <div>
                            <div className="text-[9px] uppercase text-emerald-400 font-bold mb-1">
                              ค่าและคำศัพท์สำคัญที่ปรากฏในใบผลแพทย์
                            </div>
                                ...
                                <>
                                  <li className="flex flex-col space-y-0.5">
                                    <span className="font-bold text-white">
                                      • ระดับความแข็งตับ (Stiffness): 10.4 kPa
                                    </span>
                                    <span className="text-[11px] pl-3 text-[#7a8dae]">
                                      สะท้อนการสะสมพังผืด (คล้ายแผลเป็นเล็กๆ
                                      ในเนื้อตับ)
                                      ซึ่งจำเป็นต้องติดตามประเมินกับคุณหมอประจำตัวสม่ำเสมอเพื่อไม่ให้พัฒนาต่อไป
                                    </span>
                                  </li>
                                  <li className="flex flex-col space-y-0.5">
                                    <span className="font-bold text-white">
                                      • ดัชนีไขมันเกาะตับ (CAP Status):
                                      ไขมันสะสมปานกลาง
                                    </span>
                                    <span className="text-[11px] pl-3 text-[#7a8dae]">
                                      หมายถึงมีหยดน้ำมันเล็กๆ พอกที่ตับ
                                      แนะนำให้จำกัดหรือลดการรับประทานอาหารทอด
                                      แกงกะทิ และของหวานลงอีกสักนิดนะคะ
                                    </span>
                                  </li>
                                  <li className="flex flex-col space-y-0.5">
                                    <span className="font-bold text-white">
                                      • ความมั่นใจของระบบ (Confidence): สูงมาก
                                    </span>
                                    <span className="text-[11px] pl-3 text-[#7a8dae]">
                                      วิเคราะห์โดยอิงจากสถิติเคสผู้ป่วยโรคตับเรื้อรังมากกว่า
                                      15,000 ราย
                                      ให้ค่าความแม่นยำสูงสอดคล้องกับภาพจำลองทางการแพทย์
                                    </span>
                                  </li>
                                </>
                              ) : (
                          ...
                          <div className="bg-emerald-950/30 p-3 rounded-lg border border-emerald-500/20">
                            <div className="text-[9px] uppercase text-emerald-400 font-bold mb-1 flex items-center">
                              ...
                              คู่มือการปฏิบัติตนและเมนูแนะนำ
                            </div>
                            <p className="text-xs text-white leading-relaxed">
                              {selectedModule === "smartliva"
          ...
          {activeTab === "chat" && (
            <div className="h-[calc(100vh-12rem)] flex flex-col fade-up">
              ...
              {persona === "doctor" ? (
                ...
                <div className="bg-emerald-950/30 p-2.5 rounded-xl border border-emerald-500/20 text-center mb-4">
                  <h3 className="font-bold text-xs text-white">
                    สนทนากับ HepaSage Care Coach
                  </h3>
                  <p className="text-[10px] text-emerald-400 font-medium">
                    โค้ชดูแลตับส่วนตัวที่จะดูแลคุณอย่างใกล้ชิดด้วยภาษาที่เป็นกันเอง
                  </p>
                </div>
              )}
                  ...
                  >
                    ...
                    >
                      {msg.role === "user" ? (
                        ...
                        <Bot
                          className={`w-3.5 h-3.5 ${persona === "doctor" ? "text-blue-400" : "text-emerald-400"}`}
                        />
                      )}
                    </div>
                    <div
                      className={`max-w-[80%] p-3 rounded-2xl text-xs leading-relaxed border ${
                        msg.role === "user"
                    ...
                    >
                      {msg.typing ? (
                        <span className="flex gap-1 items-center text-[#7a8dae] text-[10px] font-semibold">
                          {persona === "doctor"
                            ? "Analyzing context"
                            : "กำลังเรียบเรียงคำตอบที่เข้าใจง่าย..."}
                          {[0, 150, 300].map((d) => (
                            <span
                              key={d}
                              className={`w-1 h-1 rounded-full animate-bounce ${persona === "doctor" ? "bg-blue-400" : "bg-emerald-400"}`}
              ...
              {/* Input Area */}
              <div
                className={`bg-[#10192e] border rounded-xl p-2 flex gap-2 items-center transition-all ${
                  persona === "doctor"
                    ? "border-[#1b2f56]"
              ...
              >
                <input
                  ...
                  placeholder={
                    persona === "doctor"
                      ? chatBot === "hepasage"
                        ? "ปรึกษาข้อมูลตับหรือ FibroScan..."
                        : "Ask MARS about CCA tissue slides..."
          ...
          <div className="absolute inset-0 bg-black/70 backdrop-blur-sm z-50 flex flex-col justify-end">
            <div className="bg-[#0c1424] border-t border-[#1c3052] rounded-t-2xl p-6 space-y-4 max-h-[85vh] overflow-y-auto animate-slide-up">
              ...
              <div className="border-t border-[#1c3052] pt-4 space-y-4">
                <div>
                  <div className="text-[10px] uppercase text-[#7a8dae] font-bold mb-1.5">
                    Explainable Radiological Insights
                  </div>
                  <ul className="list-disc pl-4 text-xs text-[#a0aec0] space-y-1">
                ...
                <div className="bg-[#152444] p-3 rounded-lg border border-[#1d335c]">
                  <div className="text-[10px] uppercase text-blue-400 font-bold mb-1.5 flex items-center">
                    <Activity className="w-3.5 h-3.5 mr-1" /> Actionable
