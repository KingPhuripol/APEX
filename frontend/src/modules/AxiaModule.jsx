...
import { axiaClassify, axiaSegment, axiaDicomPreview, ApiError } from '../lib/api';
import {
  UploadCloud, AlertCircle, Brain, CheckCircle2,
  AlertTriangle, Activity, WifiOff, Info, Maximize2, MousePointer2, Layers, Search, FileText
} from 'lucide-react';
...
export default function AxiaModule({ patientId }) {
  ...
  useEffect(() => {
    fetch('/demo-dataset/manifest.json')
      .then(res => res.json())
      .then(data => {
        if (data.axia) setDemoImages(data.axia);
      })
  ...
  const runMockAnalysis = useCallback(async () => {
    ...
    setClass({
      ...
      critique: {
        thai_summary: 'พบร่องรอยการตกเลือดในสมอง (Intracranial Hemorrhage) อย่างชัดเจนในภาพถ่าย CT',
        explainable_insights: ['บริเวณ frontal lobe มีความหนาแน่นสูง (hyperdense)', 'มีแนวโน้มรบกวน ventricle ใกล้เคียง'],
        actionable_recommendations: 'แนะนำให้แจ้งศัลยแพทย์ระบบประสาททันที และเตรียมพร้อมประเมินระดับความรู้สึกตัวของผู้ป่วย'
      }
    });
    setStatus('segmenting');
    await delay(1000);
  ...
  return (
    <div className="flex h-full w-full bg-[var(--bg)]">
      ...
      <div className="flex-[3] flex flex-col border-r border-[var(--line)] bg-[#000000]">
        ...
        <div className="h-12 border-b border-[#222222] bg-[#0a0a0a] flex items-center justify-between px-4">
          ...
          <div className="flex items-center space-x-3">
            ...
            <button className="p-1.5 text-[#888888] hover:text-white transition-colors" title="Fullscreen">
        ...
        >
          {previewUrl ? (
            <div className="relative w-full h-full flex items-center justify-center">
              <img src={previewUrl} alt="Medical Scan" className="max-w-full max-h-full object-contain filter grayscale" />
              ...
              {status === 'done' && showMask && classResult?.type === 'hemorrhage' && (
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                   {/* Radar Ping rings */}
                   <div className="radar-ping-ring w-1/4 h-1/4 transform translate-x-4 -translate-y-6"></div>
                   <div className="radar-ping-ring w-1/4 h-1/4 transform translate-x-4 -translate-y-6" style={{ animationDelay: '0.75s' }}></div>
            ...
            <div className="flex flex-col items-center justify-center w-full h-full">
              <div className={`flex flex-col items-center justify-center p-8 text-center rounded-lg border-2 border-dashed transition-colors w-96 max-w-[80%] ${dragOver ? 'border-[var(--info)] bg-[var(--info-soft)]' : 'border-[#333] hover:border-[#555]'}`}>
                ...
                <div className="text-sm font-medium text-[#ccc] mb-1">Drag and drop DICOM series here</div>
                <div className="text-xs text-[#666] mb-4">or click to browse from local PACS node</div>
                <input type="file" id="dicom-upload" className="hidden" multiple accept=".dcm,.dicom,.png,.jpg" onChange={(e) => handleFiles(e.target.files)} />
                <div className="flex space-x-3">
                  <label htmlFor="dicom-upload" className="px-4 py-2 bg-[#222] hover:bg-[#333] text-white text-xs font-semibold rounded cursor-pointer transition-colors border border-[#444]">
                    Browse Files
                  </label>
                </div>
              </div>
              ...
              {demoImages.length > 0 && (
                <div className="mt-8 w-[90%] max-w-2xl">
                  <div className="text-xs text-[#888] font-bold uppercase tracking-wider mb-3 text-center">Or select a demo case</div>
                  <div className="grid grid-cols-6 gap-3">
                    {demoImages.map((url, i) => (
                      <div key={i} onClick={() => handleDemoClick(url)} className="cursor-pointer border-2 border-[#333] hover:border-[var(--info)] rounded-md overflow-hidden aspect-square transition-all hover:scale-105 bg-black">
                        <img src={url} className="w-full h-full object-cover filter grayscale" alt="Demo Case" />
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <div className="flex-[2] flex flex-col bg-[var(--surface)] border-l border-[var(--line)] overflow-y-auto">
        ...
        <div className="p-5 border-b border-[var(--line)] bg-[var(--surface-2)]">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-xs text-[var(--muted)]">AI-Assisted Brain CT Analysis</p>
            </div>
        ...
        <div className="p-5 flex-1">
          ...
          {(status === 'idle' || status === 'error') && (
            <div className="h-full flex flex-col">
              <div className="p-4 rounded-md bg-[var(--surface-2)] border border-[var(--line)] mb-6">
                <h3 className="text-sm font-bold text-[var(--text)] mb-2">Workflow Steps</h3>
          ...
          {isLoading && (
            <div className="h-full flex flex-col items-center justify-center text-center">
              ...
              <div className="font-bold text-[var(--text)] text-sm mb-1">
                {status === 'classifying' ? 'Analyzing Slices...' : 'Generating Mask & Report...'}
              </div>
              <div className="text-xs text-[var(--muted)]">AXIA is crunching the data via Cloud API</div>
            </div>
          )}
          ...
          {status === 'done' && classResult && (
            <div className="space-y-6 pb-6">
              ...
              <div className={`p-4 rounded-md border-l-4 ${tone.bg} bg-[var(--surface-2)]`}>
                <div className="flex items-center space-x-2 mb-1">
                  {tone.icon}
                  <h3 className={`font-bold text-sm ${tone.color}`}>{tone.label}</h3>
                </div>
                <div className="text-xs text-[var(--text-2)] ml-7">
                  Primary Model Confidence: {(classResult.confidence * 100).toFixed(1)}%
                </div>
              </div>
              ...
              <div className="bg-[var(--surface-2)] rounded-md border border-[var(--line)] overflow-hidden">
                <div className="bg-[var(--surface-3)] px-4 py-2 border-b border-[var(--line)] flex justify-between items-center">
                  <span className="text-[11px] font-bold text-[var(--text)] uppercase tracking-wider">Senior Critique Report</span>
                  <span className="text-[10px] bg-[var(--surface)] border border-[var(--line-strong)] px-2 py-0.5 rounded text-[var(--muted)]">Clinical Copilot</span>
                </div>
                <div className="p-4 space-y-4">
                  {classResult.critique ? (
                    <>
                      <div>
                        <div className="text-[10px] uppercase text-[var(--muted)] font-bold mb-1">Primary Finding</div>
                      ...
                      {classResult.critique.explainable_insights && (
                        <div>
                          <div className="text-[10px] uppercase text-[var(--muted)] font-bold mb-1">Explainable Insights</div>
                          <ul className="list-disc pl-4 text-xs text-[var(--text-2)] font-sans space-y-1">
                            {classResult.critique.explainable_insights.map((insight, idx) => (
                      ...
                      {classResult.critique.actionable_recommendations && (
                        <div className="bg-[var(--surface-3)] p-3 rounded border border-[var(--line)]">
                          <div className="text-[10px] uppercase text-[var(--info)] font-bold mb-1 flex items-center"><Activity className="w-3 h-3 mr-1"/> Actionable Recommendation</div>
                    ...
                    <div className="text-xs text-[var(--muted)] italic">Critique report is not available for this scan.</div>
                  )}
                </div>
              </div>
              ...
              {segResult && (
                <div>
                  <div className="text-[11px] font-bold text-[var(--text)] uppercase tracking-wider mb-2">Quantitative Metrics</div>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-[var(--surface-2)] p-3 rounded-md border border-[var(--line)]">
                      <div className="text-xs text-[var(--muted)] mb-1">Est. Volume</div>
                      <div className="text-lg font-mono font-bold text-[var(--text)]">
                        {segResult.volume != null ? segResult.volume.toFixed(1) : '—'}<span className="text-xs font-sans font-normal text-[var(--muted)] ml-1">mL</span>
                      </div>
                    </div>
                    <div className="bg-[var(--surface-2)] p-3 rounded-md border border-[var(--line)]">
                      <div className="text-xs text-[var(--muted)] mb-1">Midline Shift</div>
                      <div className="text-lg font-mono font-bold text-[var(--text)]">
                        {segResult.midlineShift != null ? segResult.midlineShift.toFixed(1) : '—'}<span className="text-xs font-sans font-normal text-[var(--muted)] ml-1">mm</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {/* Confidence Details */}
              <div>
                <div className="text-[11px] font-bold text-[var(--text)] uppercase tracking-wider mb-2">Confidence Scores</div>
                <div className="bg-[var(--surface-2)] p-4 rounded-md border border-[var(--line)]">
                  <ConfidenceBar value={classResult.confidence} label="Overall Confidence" />
                  {classResult.stage1Score && <ConfidenceBar value={classResult.stage1Score} label="Hemorrhage Detector" colorClass="bg-[var(--danger)]" />}
                  {classResult.stage2Score && <ConfidenceBar value={classResult.stage2Score} label="Ischemic Detector" colorClass="bg-[var(--warn)]" />}
                </div>
              </div>
      ...
      <MedicalReportModal 
        ...
        analysisResult={classResult ? {
          stage4_formatter: {
            ...
            explainable_insights: classResult.critique?.explainable_insights,
            actionable_recommendations: classResult.critique?.actionable_recommendations
          }
        } : null}
        patientId={patientId}
        moduleName="axia"
