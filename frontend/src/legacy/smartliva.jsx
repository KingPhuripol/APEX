...
import { smartlivaPredict, smartlivaChat, ApiError } from "../lib/api.js";
...
const INITIAL_CHAT = [
  {
    who: "ai",
    text: "Hello! I'm HepaSage, your AI hepatologist assistant. Upload a liver ultrasound image to begin analysis, or ask me anything about liver health.",
  },
];
...
function SmartLiva({ running: globalRunning }) {
      ...
      if (err instanceof ApiError && err.status === 503) {
        ...
        setTimeout(() => {
          setResult({
            ...
            analysis_notes:
              "Hypoechoic nodule with irregular margin noted in right lobe. Pattern consistent with early HCC. Recommend contrast-enhanced CT/MRI.",
          });
          setStatus("done");
          setChat((prev) => [
            ...prev,
    ...
    try {
      ...
      setTimeout(() => {
        setChat((c) => {
          ...
          next.push({
            who: "ai",
            text: "Based on the F2 fibrosis staging, I'd recommend multiphasic contrast-enhanced MRI within 2 weeks.",
          });
          return next;
        });
      }, 1000);
  ...
  const fetchFromPACS = async () => {
    ...
    try {
      ...
      setResult({
        ...
        fibrosis_stage: "F2",
        fibrosis_confidence:
          pipeline.stage2_analysis.raw_findings?.confidence || 0.74,
        lesion_label: "HCC",
        lesion_confidence: 0.68,
  ...
  const isRunning =
    ["fetching", "anonymizing", "queued", "analyzing"].includes(status) ||
    globalRunning;
  ...
  return (
    <div className="fade-up space-y-4">
      <PageHeader
        ...
        title={
          <>
            Liver lesion characterization with{" "}
            <span className="accent-text">conformal prediction</span>
          </>
        }
        ...
        right={
          <>
            <Badge tone="ok">
              <Icon name="shield-check" size={11} /> {coverage}% coverage
            </Badge>
            {isMockMode && (
              <Badge tone="warn">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-4">
            ...
            <div className="p-10 flex flex-col items-center justify-center min-h-[400px]">
              <div className="w-full max-w-lg border rounded-xl p-8 flex flex-col items-center justify-center text-center bg-slate-50">
                ...
                <div className="text-[13px] text-slate-500 mt-1 mb-6">
                  Pull current ultrasound directly from hospital VNA
                </div>
            ...
            <>
              <div className="px-4 py-3 border-b hairline flex items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  <Badge tone="accent">
                    <Icon name="audio-waveform" size={11} /> B-mode · CLAHE
                  </Badge>
              ...
              <div className="relative aspect-[4/3] overflow-hidden viewer-dark">
                ...
                {result && (
                  <div className="absolute top-3 right-3 font-mono whitespace-nowrap text-[10.5px] text-teal-200/90 text-right space-y-0.5 pointer-events-none bg-slate-900/50 p-2 rounded-md backdrop-blur-sm">
                    <div className="text-[12px] font-semibold">
                      SmartLiva Prediction
                    </div>
                    <div>
                      Stage: {result.fibrosis_stage} (
                      {(result.fibrosis_confidence * 100).toFixed(1)}%)
                    </div>
                    {result.lesion_confidence > 0 && (
                      <div>
                        Lesion: {result.lesion_label} (
        ...
        {status !== "idle" && status !== "error" && (
          <div className="xl:col-span-5 space-y-4">
            <Card
              eyebrow="Statistical safety"
              title="Conformal fibrosis staging"
              right={
                <Badge tone="accent">
                  <Icon name="shield-check" size={11} /> {coverage}% certified
                </Badge>
              }
            >
              ...
              <div className="mt-3 text-[12.5px] t-secondary leading-relaxed">
                The model emits a{" "}
                <span className="font-semibold accent-text">set</span>, not a
                single label. Set size shrinks as imaging quality improves.
              </div>
              <div className="mt-3 grid grid-cols-2 gap-3">
                <Stat
                  label="Top-1 prob"
                  value={
                    result ? result.fibrosis_confidence.toFixed(2) : "0.74"
                  }
                  sub={stage}
                  size="sm"
                />
              ...
              <div className="mt-3 flex items-center gap-2">
                ...
                <div className="font-mono text-[12px] t-primary w-10 text-right font-semibold">
                  {coverage}%
                </div>
              </div>
            </Card>
            ...
            <Card
              eyebrow="Transient elastography"
              title="Liver stiffness · kPa"
              right={
                <Badge tone={stiffness > 9.5 ? "warn" : "ok"}>
                  {stage} range
                </Badge>
              }
            >
              <div className="grid grid-cols-[180px_1fr] gap-4 items-center">
                ...
                <div>
                  <div className="text-[12px] t-secondary leading-relaxed">
                    Predicted from B-mode ultrasound patterns using deep
                    regression.
                  </div>
                  <div className="mt-3 space-y-1.5 text-[12px]">
                    ...
                    <Threshold
                      label="F2"
                      range="7.1 – 9.5"
                      active={stiffness > 7 && stiffness <= 9.5}
                    />
                    <Threshold
                      label="F3"
                      range="9.6 – 12.5"
                      active={stiffness > 9.5 && stiffness <= 12.5}
                    />
                    <Threshold
                      label="F4 cirrhosis"
                      range="≥ 12.5"
                      active={stiffness > 12.5}
                    />
                  </div>
                </div>
              </div>
            ...
            <Card
              eyebrow="HepaSage AI"
              title="Clinical reasoning chat"
              padded={false}
            >
              <div
                ref={chatScroll}
                className="px-4 py-3 max-h-[220px] overflow-y-auto space-y-3"
              >
                {chat.map((m, i) => (
                  <ChatMsg key={i} m={m} />
                ))}
      ...
      {all.map((k) => {
        ...
        return (
          ...
          >
            ...
            <div className="text-[9.5px] uppercase tracking-wider opacity-80 mt-0.5">
              {labels[k]}
            </div>
          </div>
        );
...
function Threshold({ label, range, active }) {
  return (
    <div
      className={`flex items-center justify-between gap-2 px-2.5 py-1.5 rounded-md border accent-transition whitespace-nowrap
      ${active ? "border-[color:var(--accent)] bg-[color:var(--accent-soft)]" : "border-transparent bg-slate-50"}`}
    >
      <span className={active ? "font-semibold accent-text" : "t-primary"}>
        {label}
      </span>
        ...
        {m.typing ? (
          <span className="inline-flex items-center gap-1 t-secondary">
            ...
            <span className="inline-flex gap-0.5">
              ...
              <span
                className="w-1 h-1 rounded-full bg-[color:var(--accent)] pulse-soft"
  ...
  return (
    <svg viewBox="0 0 800 600" className="w-full h-full">
      <defs>
        ...
        <clipPath id="usFan">
          <polygon points="400,0 760,580 40,580" />
        </clipPath>
      </defs>
      ...
      <g clipPath="url(#usFan)">
        ...
        <g opacity="0.9">
          <path
            d="M150,260 Q300,290 540,330 Q610,345 700,340"
            stroke="#0b1f2e"
            strokeWidth="14"
            fill="none"
            strokeLinecap="round"
        ...
        {Array.from({ length: 14 }, (_, i) => {
          ...
          return (
            <g key={i}>
              ...
              />
              {i % 2 === 0 && i > 0 && (
                <text x="754" y={y + 3} textAnchor="end">
                  {i} cm
                </text>
              )}
            </g>
