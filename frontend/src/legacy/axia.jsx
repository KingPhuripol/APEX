...
import { Icon, Badge, Stat, ProgressBar, Btn, Card, PageHeader, PatientStrip, StatusDot } from './ui.jsx';
import { axiaClassify, axiaSegment, coreOrchestrate, ApiError } from '../lib/api.js';
...
function Axia({ running: globalRunning }) {
  ...
  const isRunning = ['fetching', 'anonymizing', 'queued', 'classifying', 'segmenting'].includes(status) || globalRunning;
  ...
  return (
    <div className="fade-up space-y-4">
      <PageHeader
        ...
        title={
          result && result.type === 'hemorrhage' ? (
            <>Acute intracranial hemorrhage — <span className="accent-text">right MCA territory</span></>
          ) : result ? (
        ...
        right={
          <>
            {result && result.volume > 10 && <Badge tone="danger" className="flash-warn"><Icon name="siren" size={11} /> Critical · Code Stroke</Badge>}
            ...
            <Btn icon={isRunning ? "loader" : "refresh-cw"} size="sm" onClick={runAnalysis} disabled={!files.length || isRunning}>
      ...
      <PatientStrip items={[
        ...
        { k: "Symptom onset", v: <span className="text-[#c1272d]">T + 22 min</span> },
          ...
          {status === 'idle' || status === 'error' ? (
            <div className="p-10 flex flex-col items-center justify-center min-h-[500px]">
              <div className="w-full max-w-lg border rounded-xl p-8 flex flex-col items-center justify-center text-center bg-slate-50">
                ...
                <div className="text-[13px] text-slate-500 mt-1 mb-6">Pull current study directly from hospital VNA</div>
                ...
                <div 
                  ...
                  onClick={() => document.getElementById('axia-file-upload').click()}
                  onDragOver={(e) => e.preventDefault()}
                  onDrop={(e) => { e.preventDefault(); handleFiles(e.dataTransfer.files); }}
                >
                  <input id="axia-file-upload" type="file" multiple className="hidden" accept=".dcm,.dicom,image/*" onChange={e => handleFiles(e.target.files)} />
                  ...
                  <div className="text-[11.5px] text-slate-500 mt-0.5">DICOM or images supported</div>
                </div>
                ...
                {files.length > 0 && (
                  <div className="mt-4 w-full text-left">
                    ...
                    <Btn className="w-full" icon="sparkles" onClick={runAnalysis}>Run AXIA Analysis</Btn>
                  </div>
                )}
              </div>
            ...
            <>
              ...
              <div className="relative grid grid-cols-[1fr_64px] viewer-dark">
                <div className="relative aspect-square viewer-grid" style={{ background: "radial-gradient(circle at center, #131c2c 0%, #07101b 80%)" }}>
                  <BrainCT slice={slice} total={total} overlay={overlay} window={windowing} result={result} />
                    ...
                    <div className="absolute inset-0 overflow-hidden pointer-events-none bg-black/40 flex items-center justify-center">
                      <div className="text-white flex flex-col items-center gap-3 font-medium">
                        ...
                        {status === 'fetching' ? 'Fetching from PACS...' :
                         ...
                         status === 'classifying' ? 'Classifying scan...' : 'Segmenting region...'}
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="border-l border-slate-800 overflow-y-auto" style={{ maxHeight: 520 }}>
                  {Array.from({ length: total }, (_, i) => i + 1).map((n) => {
                    ...
                    const hot = result && result.type === 'hemorrhage' && n >= 14 && n <= 20;
                    return (
                      <button
                        key={n}
                        onClick={() => setSlice(n)}
              ...
              <div className="px-4 py-3 border-t hairline bg-white">
                <div className="flex items-center gap-3">
                  <button onClick={() => setSlice(Math.max(1, slice - 1))} className="w-8 h-8 rounded-md grid place-items-center border hairline-strong hover:bg-slate-50 t-secondary"><Icon name="chevron-left" size={14} /></button>
                  <input
                    type="range"
                  ...
                  />
                  <button onClick={() => setSlice(Math.min(total, slice + 1))} className="w-8 h-8 rounded-md grid place-items-center border hairline-strong hover:bg-slate-50 t-secondary"><Icon name="chevron-right" size={14} /></button>
                  <div className="font-mono text-[13px] t-primary tabular-nums w-16 text-right font-semibold">{slice}<span className="t-muted"> / {total}</span></div>
                </div>
              </div>
            </>
          )}
        ...
        <div className="xl:col-span-5 space-y-4">
          <Card eyebrow="Quantification" title="Automated clinical metrics">
            <div className="grid grid-cols-2 gap-3">
              ...
              <MetricBlock label="Midline shift" value={midline} unit="mm" tone={midline >= 5 ? "danger" : midline >= 2 ? "warn" : "ok"} max={10} note={midline >= 5 ? "Significant mass effect" : midline >= 2 ? "Mild" : "Within normal limits"} />
          ...
          <Card eyebrow="Imaging" title="ASPECTS · 10-point score" right={<Badge tone="danger">7 / 10</Badge>}>
            <Aspects />
          ...
          {result && result.volume > 30 && (
            <div className="rounded-md border flash-warn px-4 py-3 flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[12.5px] text-[#7a1b1f] leading-relaxed mt-0.5">Hemorrhage volume {hemoVol} mL with {midline} mm midline shift exceeds operative threshold. Suggest urgent decompressive craniectomy review.</div>
  ...
  return (
    <div className="surface-2 rounded-md p-3">
      ...
      <div className="mt-1 flex items-baseline gap-1.5">
        <div className="text-[22px] font-semibold tabular-nums" style={{ color: colors[tone] }}>{typeof value === 'number' ? value.toFixed(1) : value}</div>
        ...
        {isHemo && inHemo && overlay && (
          <g>
            ...
            <g fontFamily="ui-monospace, monospace" fontSize="10" fill="#fda4af">
              ...
              <text x="323" y="173" fill="#fcd34d">vol {(result.volume - Math.abs(slice - 17) * 4.2).toFixed(1)} mL</text>
            </g>
          </g>
        )}
      </g>
